...
conditional_code = 2  # dim of conditional code
img_channel = 3  # channel of generated image
init_channel = 128  # control the initial Conv channel of the Generator and Discriminator
...
def load_data():
    ...
    # dataset
    dataset = torchvision.datasets.ImageFolder(root='./celeba/', transform=data_transform)

    return DataLoader(dataset, batch_size, True, num_workers=workers)
...
class Generator(nn.Module):
    def __init__(self):
        ...
        self.deconv1_1 = nn.Sequential(  # input for latent coda
            nn.ConvTranspose2d(latent, init_channel * 4, 4, 1,0),
            nn.BatchNorm2d(init_channel * 4),
            nn.ReLU(),
        )

        self.deconv1_2 = nn.Sequential(  # input for conditional code
            nn.ConvTranspose2d(conditional_code, init_channel * 4, 4, 1,0),
            nn.BatchNorm2d(init_channel * 4),
            nn.ReLU(),
        )

        self.deconv2 = nn.Sequential(
            nn.ConvTranspose2d(init_channel * 8, init_channel * 4, 4, 2, 1),
            nn.BatchNorm2d(init_channel * 4),
            nn.ReLU(),
        )

        self.deconv3 = nn.Sequential(
            nn.ConvTranspose2d(init_channel * 4, init_channel * 2, 4, 2, 1),
            nn.BatchNorm2d(init_channel * 2),
            nn.ReLU(),
        )

        self.deconv4 = nn.Sequential(
            nn.ConvTranspose2d(init_channel * 2, init_channel, 4, 2, 1),
            nn.BatchNorm2d(init_channel),
            nn.ReLU(),
        )

        self.deconv5 = nn.Sequential(
            nn.ConvTranspose2d(init_channel, img_channel, 4, 2, 1),
            nn.Tanh()
        )
...
class Discriminator(nn.Module):
    def __init__(self):
        ...
        self.conv1_1 = nn.Sequential(  # input for image
            nn.Conv2d(img_channel, init_channel // 2, 4, 2, 1),
            nn.LeakyReLU(slope)
        )

        self.conv1_2 = nn.Sequential(  # input for onehot code
            nn.Conv2d(conditional_code, init_channel // 2, 4, 2, 1),
            nn.LeakyReLU(slope)
        )

        self.conv2 = nn.Sequential(
            nn.Conv2d(init_channel, init_channel * 2, 4, 2, 1),
            nn.BatchNorm2d(init_channel * 2),
            nn.LeakyReLU(slope)
        )
    ...
    for epoch in range(epoch_num):
        for step, (real_data, target) in enumerate(data_loader, 1):
            ...
            if step % 100 is 0:
                ...
                generate_img = torchvision.utils.make_grid((0.5 * net_g(get_noise(real_data.size(0)), fake_label).data.cpu() + 0.5))
                generated = generate_img.permute(1, 2, 0).numpy()
                plt.imshow(generated)
                plt.pause(0.01)
                plt.imsave('./generated/cdcgan/' + str(number) + '.png', generated)
                # cv.imwrite('./generated/cdcgan/' + str(number) + '.png', generated * 255)
